...
Two defects sit directly on that untested path and are expected to fire. Fix them
**only if they do** fire:
...
- **E4** — `app/api/gmail/draft/route.ts:87-106`. `draftId` is captured at line 87 then
  ...
  success message.
  Fix: only overwrite `draftLink` when truthy; use `draftId` or `drafts.get` for the link.
