...
| --- | --- | --- | --- | --- |
| **Q1** | What legal entity name and physical postal address go in the footer? Required by statute; there is no default. **The answer must also confirm there is only one** — if Mangood and MichiKanji send as different legal entities, one app-wide footer is statutorily wrong on one of them and REQ-06 has to change shape before CRM-104 starts. Current evidence says one entity: a single Workspace tenant with a neutral primary domain, the product domains attached as aliases. | REQ-06 | Product | Before CRM-104 starts |
...
| Per-campaign footer identity. Today it is one `CRM_SENDER_*` pair for the whole app, which is correct while one legal entity sends everything (Q1). | A **second legal entity** starts sending, or a campaign needs a different postal address. Note the fix is then the parked **Product tier** (roadmap D16), not columns on `Project`: two Mangood campaigns share one sender, so a per-project column would store the same entity twice. |
